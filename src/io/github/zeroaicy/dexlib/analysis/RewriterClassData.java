...
public class RewriterClassData implements Comparable<RewriterClassData> {
	...
	public RewriterClassData(String confusevt, String renamed) {
		this.confusevt = confusevt;
		this.renamed = renamed;
		if( renamed != null && renamed.contains("\\")){
			String format = String.format("警告⚠️，严重警告⚠️，新类名 出现 '\\' %s  -> %s", confusevt, renamed);
	...
	public boolean notChange() {
		return notChangeClassName() && !hasFieldData() && !hasMethodData();
	}
	...
	public void addFieldData(RewriterClassData.FieldData fieldData) {
		if (fieldDatas == null) {
			fieldDatas = new HashMap<>();
		}
		if (!fieldDatas.containsKey(confusevt)) {
			fieldDatas.put(fieldData.confusevt, fieldData);
		}
	}

	public boolean hasMethodData() {
		return this.methodDataMap != null && !this.methodDataMap.isEmpty();
	}
	public boolean hasFieldData() {
		return fieldDatas != null && !fieldDatas.isEmpty();
	}
	...
	public MethodData addMethodData(MethodData methodData) {
		...
		this.methodDataMap.put(methodSignature, methodData);
	...
	public MethodData addMethodData(String confusevt, String parameters, String renamed) {
		...
		RewriterClassData.MethodData methodData = methodDataMap.get(confusevt + methodParametersSignature);
		if (methodData == null) {
	...
	public static class MethodData implements Comparable<MethodData> {
		...
		public MethodData(String confusevt, String parametersSignature, String renamed) {
			...
			if( renamed != null && renamed.contains(" ")){
				String format = String.format("严重警告⚠️:方法新名有空格，-> %s : %s", confusevt, renamed);
				System.out.println(format);
			}
		}
